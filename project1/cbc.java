...
public class cbc {
    ...
    public String cbc_enc(String init, String input, byte[] data) {
        int round = 10;
        int inputLength = 0;
        ...
        while(inputLength < newInput.length()) {
            ...
            for (int i = 0; i < divideInput.length(); i++) {
                sb1.append(charOf(bitOf(divideInput.charAt(i)) ^ bitOf(init.charAt(i))));
            }
            ...
            while (index < sb1.length()) {
                strings.add(sb1.substring(index, Math.min(index + 48,sb1.length())));
                index += 48;
            } 
